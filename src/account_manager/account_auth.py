...
class AccountAuth:
    ...
    def get_current_version(self):
        return requests.get(
            "https://valorant-api.com/v1/version", timeout=HTTP_TIMEOUT
        ).json()["data"]
    ...
    def auth_account(self, username=None, password=None, cookies=None):
        ...
        r = self.session.post('https://auth.riotgames.com/api/v1/authorization', json=data, headers=self.headers, timeout=HTTP_TIMEOUT)
        ...
        if username != None and password != None:
            ...
            r = self.session.put("https://auth.riotgames.com/api/v1/authorization", json=body, headers=self.headers, timeout=HTTP_TIMEOUT)
            #check for 2fa
        ...
        r_entitlements = self.session.post('https://entitlements.auth.riotgames.com/api/token/v1', headers={'Authorization': 'Bearer ' + access_token} | self.headers, json={}, timeout=HTTP_TIMEOUT)
        entitlements_token = r_entitlements.json()['entitlements_token']
        self.auth_headers.update({
            'Authorization': f"Bearer {access_token}",
            'X-Riot-Entitlements-JWT': entitlements_token})
        r = requests.put("https://riot-geo.pas.si.riotgames.com/pas/v1/product/valorant", headers={'Authorization': 'Bearer ' + access_token}, json={"id_token": id_token}, timeout=HTTP_TIMEOUT)
        self.region = r.json()["affinities"]["live"]
        r = requests.post("https://auth.riotgames.com/userinfo", headers={'Authorization': 'Bearer ' + access_token}, timeout=HTTP_TIMEOUT)
    ...
    def get_latest_season_id(self):
        ...
        if self.content is None:
            self.content = requests.get(f"https://shared.{self.region}.a.pvp.net/content-service/v3/content", headers=self.auth_headers, verify=False, timeout=HTTP_TIMEOUT)
    ...
    def get_account_data(self):
        ...
        r_mmr = requests.get(f"https://pd.{self.region}.a.pvp.net/mmr/v1/players/{self.puuid}", headers=self.auth_headers, verify=False, timeout=HTTP_TIMEOUT)
        ...
        name = requests.put(f"https://pd.{self.region}.a.pvp.net/name-service/v2/players", headers=self.auth_headers, json=[self.puuid], timeout=HTTP_TIMEOUT).json()
        name = name[0]["GameName"] + "#" + name[0]["TagLine"]
        r_account_xp = requests.get(f"https://pd.{self.region}.a.pvp.net/account-xp/v1/players/{self.puuid}", headers=self.auth_headers, verify=False, timeout=HTTP_TIMEOUT)
        level = r_account_xp.json()["Progress"]["Level"]
        contracts = requests.get("https://valorant-api.com/v1/contracts", timeout=HTTP_TIMEOUT)
        contracts = [a for a in contracts.json()["data"] if a["content"]["relationType"] == "Season"]
        bp = contracts[-1]
        r_contracts = requests.get(f"https://pd.{self.region}.a.pvp.net/contracts/v1/contracts/{self.puuid}", headers=self.auth_headers, verify=False, timeout=HTTP_TIMEOUT)
